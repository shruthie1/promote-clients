...
async function overwriteFile(branch) {
  ...
  try {
    const result = await cloudinary.v2.uploader.upload(localFilePath, {
      ...
      overwrite: true,
      invalidate: true,
      public_id: `index-pmc-${branch}.js`
    });
    console.log(result);

    const url = `https://uptimechecker2.glitch.me/builds`;
    const bodyData = {};
    bodyData[`promotion-clients-new-main-2`] = `https://res.cloudinary.com/${process.env.CL_NAME}/raw/upload/v${result.version}/${result.public_id}`
